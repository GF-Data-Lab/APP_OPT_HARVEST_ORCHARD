...
def make_bin_sets(arboles_cara, a_x=4.0, d_y=2.0, gap_x=2.0, gap_y=2.0, y_off=-5.0):
    n = len(arboles_cara)
    xc = np.arange(n) * a_x
    cx = (n - 1) * a_x / 2
...
def run_sim(ppface: int, layout: str) -> float:
    ...
    for lst in face_to_idx.values():
        lst.sort(key=lambda ix: trees[ix][1])
    ...
    for fc in workers_faces:
        lst = face_to_idx[fc]
        c = ptr.get(fc, 0)
        worker_pos.append(tuple(trees[lst[c]]))
        ptr[fc] = c + 1

    # estado
    w_time = np.zeros(workers)
    ...
    while remain and np.isfinite(w_time).any():
        ...
        # bines secuenciales
        b = open_bin
        if bin_start[b] and now - bin_start[b] >= BIN_TIME_LIMIT:
            b += 1
        if b < n_bins and bin_load[b] >= BIN_CAP:
            b += 1
        if b < n_bins and b != open_bin:
            open_bin = b
        if open_bin >= n_bins:
            w_time[w] = now + 5
        ...
        if bin_start[open_bin] is None:
            bin_start[open_bin] = now
        walk = dist / speed[w]
        harv = fatigue(now)
        tot = walk + harv
        fin = now + tot
        w_time[w] = fin
        worker_pos[w] = tuple(bins[open_bin])
        bin_load[open_bin] += 1
...
plt.grid(alpha=0.3)
plt.legend()
plt.xlabel("Personas por cara")
plt.ylabel("Horas para terminar")
