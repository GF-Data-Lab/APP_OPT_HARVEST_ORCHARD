...
# ────────────────────────────── CONSTANTS
P = dict(dt=5, horizon=360, picker_rate15=10, tree_spacing=2,
         bin_cap=300, bucket_cap=10, walk_speed=60)
...
def equidistant_bins(rows: int, L: int, k: int):
    ...
    return np.column_stack([
        np.tile(xs, rows)[:k],
        np.repeat(np.arange(rows), per_row)[:k]
    ])
...
bins_xy = robust_layout(rows, L, k_bins, density) if placement == 'Optimised' else equidistant_bins(rows, L, k_bins)
if bins_xy.size == 0:
    st.error('❌ No layout generated.'); st.stop()
...
st.dataframe(df.style.apply(lambda r: ['background-color:#d6f5d6' if r.km_pp == best.km_pp else '' for _ in r], axis=1))
fig = px.line(df, x='pickers', y='km_pp', color='strategy', markers=True, title='Km/picker vs pickers')
fig.add_scatter(x=[best.pickers], y=[best.km_pp], mode='markers', marker=dict(size=12, symbol='star', color='gold'), name='Best')
st.plotly_chart(fig, use_container_width=True)

st.success(f"Best → {best.strategy} with {best.pickers} pickers: {best.km_pp:.2f} km/picker • {best.minutes} min")

fig_bins = px.scatter(pd.DataFrame(bins_xy, columns=['x', 'row']), x='x', y='row', height=450, range_x=[0, L], range_y=[rows, -1])
