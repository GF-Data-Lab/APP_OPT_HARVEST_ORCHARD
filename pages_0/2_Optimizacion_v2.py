...
-----------------------------
1. **Dynamic orchard blocks by team** – The app now scans for JSON files
   called ``equipo<N>.json`` (e.g. ``equipo4.json``).  Each file must
   contain a top‑level key ``"ORCHARD_BLOCKS"`` that holds the list of
   orchard‑block dictionaries you provided.  This replaces the previous
   ``equipo<N>.txt`` mechanism.
2. **Two‑step sidebar filter** – First choose an *Equipo* (file), then a
   specific *Bloque* from that file.  Selecting a block automatically
   fills ``trees_per_face`` and shows its metadata.  You can still pick
   **Entrada manual** to enter row counts directly.
3. All existing parameters (row spacing, target per bin, etc.) remain
   editable.  Errors are surfaced gracefully if files are missing or
   malformed.
"""
    ...
    *Scans for files matching ``equipo*.json``.*
    ...
    for path in Path(directory).glob("equipo*.json"):
        try:
            ...
            # Skip unreadable or malformed files but log to Streamlit later
            team_data[path.stem] = exc  # store the error for user feedback
    return team_data
...
def blocks_to_trees_per_face(block: dict) -> List[int]:
    ...
    Assumes *hileras* is a list of ``[row_number, num_trees]`` where
    rows start at 1. Missing rows will be zero‑filled.
    """
...
# ────────────────────────────────────────────────────────────────
# 1 · Core algorithm utilities (unchanged)
# ────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────
# 2 · Plotting helper (unchanged)
# ────────────────────────────────────────────────────────────────

# <BEGIN plot_orchard definition>
