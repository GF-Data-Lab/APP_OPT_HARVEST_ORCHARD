...
def cargar_datos_excel(uploaded_file):
    ...
    try:
        ...
        numeric_columns = [
            ...
            "N° HILERAS",
            "ESTIMACION KG/HÁ",
            "KILOS COSECHADOS FLOREO X PLANTA",
        ...
        st.error(f"Error al cargar el archivo Excel: {e!s}")
        return None
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if df_cosecha is not None:
            ...
            df_cosecha["CECO_CUARTEL"] = (
                df_cosecha["CECO"].astype(str) + " - Cuartel " + df_cosecha["CUARTEL"].astype(str)
            )
    ...
    if uploaded_file is not None and df_cosecha is not None:
        ...
        campo = st.sidebar.text_input(
            "Nombre del Campo",
            f"CECO {fila['CECO']} Cuartel {fila['CUARTEL']}",
        )
    ...
    if st.sidebar.button("🚀 Generar Optimización", type="primary"):
        try:
            ...
            with st.spinner("Generando mapas y optimizaciones..."):
                ...
                H_H_1 = [
                    row[1]
                    for group in d_barrer.values()
                    for row in group
                    if len(row) >= 3 and row[2] == "bin"
                ]
                H_H_2 = [
                    row[1]
                    for group in d_barrer_2.values()
                    for row in group
                    if len(row) >= 3 and row[2] == "bin"
                ]
                H_H_3 = [
                    row[1]
                    for group in d_barrer_3.values()
                    for row in group
                    if len(row) >= 3 and row[2] == "bin"
                ]
                H_H_4 = [
                    row[1]
                    for group in d_barrer_4.values()
                    for row in group
                    if len(row) >= 3 and row[2] == "bin"
                ]
                H_H_F = [
                    row[1]
                    for group in d_floreo.values()
                    for row in group
                    if len(row) >= 3 and row[2] == "bin"
                ]
    ...
    if "resultados" in st.session_state:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["📊 Resumen", "🌲 Floreo", "🍎 Barrer", "📄 Archivos Generados"],
        )
        ...
        st.info(
            "👈 Puedes cargar un archivo Excel del plan de cosecha o configurar los parámetros manualmente en el panel lateral",
        )
