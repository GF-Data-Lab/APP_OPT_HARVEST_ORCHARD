...
def rectangle_bounds(l, M, alpha, beta, r, a):
    return [(alpha+i*(r+a), alpha+i*(r+a)+r, beta, beta+l)
            for i in range(len(M))]

# ----------------------------------------------------------
# 2. Utilidades
# ----------------------------------------------------------
def is_inside_rect(x,y,rect): xl,xr,yb,yt=rect; return xl<=x<=xr and yb<=y<=yt
...
def balanced_kmeans(pts, k, max_iter=100):
    n,_=pts.shape; cap=int(np.ceil(n/k))
    ...
    for _ in range(max_iter):
        ...
        for c in over:
            ...
            for i in idxs[np.argsort(-d)]:
                if counts[c]<=cap: break
                nearest=np.argsort(np.linalg.norm(pts[i]-centers,axis=1))
                for t in nearest:
                    if counts[t]<cap:
                        labels[i]=t; counts[c]-=1; counts[t]+=1; break
        for c in range(k): centers[c]=pts[labels==c].mean(axis=0)
    return labels,centers

# ----------------------------------------------------------
# 3. Pipeline principal
# ----------------------------------------------------------
def place_bins_balanced(l,M,alpha,beta,r,a,k,entries=None,
                        lam=10,eps_factor=0.05):
    if entries is None: entries=[]
    trees = midpoints_rectangles(l,M,alpha,beta,r,a)
    rects = rectangle_bounds(l,M,alpha,beta,r,a)

    pts  = trees.tolist()
    for x,y,n in entries: pts.append((x,y))          # sin pesos en versión balanced
    pts=np.array(pts)
...
def plot_scene(pts,lbl,C,C_adj,rects,k):
    fig,ax=plt.subplots(figsize=(10,4)); ax.set_aspect('equal')
    draw_rects(ax,rects)
    for c in range(k): ax.scatter(*pts[lbl==c].T,s=15,label=f'Cluster {c}')
    ...
    ax.set_xlabel('x'); ax.set_ylabel('y'); ax.set_title('Bines balanceados')
...
def parse_args():
    ...
    for name in ['l','alpha','beta','r','a']: P.add_argument(f'--{name}',type=float)
    P.add_argument('--M',type=str); P.add_argument('--k',type=int)
...
if __name__=="__main__":
    cli=parse_args()
    if cli:
        params={'l':cli.l,'M':ast.literal_eval(cli.M),
                'alpha':cli.alpha,'beta':cli.beta,
                'r':cli.r,'a':cli.a,'k':cli.k,
    ...
    pts,lbl,C,C_adj,rects = place_bins_balanced(
        params['l'],params['M'],params['alpha'],params['beta'],
        params['r'],params['a'],params['k'],params['extra'])
    ...
    if input("\n¿Comparar con bines propios? [s/N]: ").strip().lower()=='s':
        ...
        if len(manual)!=params['k']:
            print("⚠️  Cantidad incorrecta, fin."); sys.exit()
        manual=np.array(manual)
        d_man=distances_to_bins(pts,lbl,manual)
        print("\nMedias por cluster:")
        for c in range(params['k']):
            print(f"Cluster {c+1}: auto={d_auto[lbl==c].mean():.2f} | manual={d_man[lbl==c].mean():.2f}")
